...
class FaceAlignment:
    def __init__(self, landmarks_type, network_size=NetworkSize.LARGE,
        ...
        assert len(self.workers) > 1, "Must be 2 or more workers."
        ...
        ### Encrypt model.
        self.face_alignment_net = self.face_alignment_net.fix_precision().share(*workers)
    ...
    def get_landmarks_from_image(self, image_or_path, detected_faces=None):
        ...
        # Drop alpha channel. Left RGB channels 
        input_image = image[..., ::-1].copy()
        # Convert to tensor.
        input_image = torch.from_numpy(input_image).float().to(self.device)
        # Encrypt input image
        input_image = input_image.fix_precision().share(*self.workers)
        ...
        if detected_faces is None:
            ...
            print("Detected Faces Ecncrypted: ", detected_faces, type(detected_faces))
            detected_faces = detected_faces.get().float_precision()
            print("Detected Faces : ", detected_faces, type(detected_faces))
        ...
        for i, d in enumerate(detected_faces):
            ...
            # Encrypt face 
            inp = inp.fix_precision().share(*self.workers)
